...
$X = Read-Host "
Esse script executa as seguintes ações. Migração On-Boarding (move a caixa para o O365), migração Off-Boarding (traz a caixa do O365) e finaliza um usuário já sincronizado.
...
if($x -eq 1){
    ...
    New-MoveRequest -Remote -Identity $Identity -RemoteHostName 'webmail.grupoccr.com.br' -RemoteCredential $AdminCred -TargetDeliveryDomain 'grupoccr.mail.onmicrosoft.com' -BadItemLimit 100
